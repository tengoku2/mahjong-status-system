...
- まどろみ通常卓の正式ルール名
- 大会・リーグ・通常会の区分名
- Event の作成を Discord で行うか、管理用 Web で行うか
...
1. この要件定義を確定する
2. RuleSet / Event の Prisma schema 案を作る
3. 既存 `tournamentName` との互換方針を決める
4. `/api/tanki/v2/matches` の payload を確定する
5. watcher を v2 payload に対応させる
6. 飛びなし TANKI パッチの検証計画を作る
7. VRChat テキストビュー API の最小版を作る

初期実装の優先度は、RuleSet / Event、TANKI v2 取り込み、飛びなし検証の順とする。
牌譜 JSON と WebApp は、局単位データの安定後に進める。
