...
相手PC側のBotリポジトリ配置先は、今後この固定パスで運用します。

```text
D:\白鳳会\mahjong-status-system-main\mahjong-status-system-main
```

以後のPowerShell例もこのパスを前提にしています。
...
- 先にVRChatへ入る
- その後で監視ツールを起動する
- VRChatを再起動しても、監視ツールは最新ログへ自動追従します

### 4. ローカル確認モードで監視を起動する

PowerShell でも起動できますが、通常は `.cmd` をダブルクリックで十分です。
...
起動後は watcher が最新の `output_log` を自動追従します。

毎回の入力を減らしたい場合は、一度だけデスクトップショートカットを作成してください。
...
```powershell
$latest = Get-ChildItem "C:\Users\USERNAME\AppData\LocalLow\VRChat\VRChat" -Filter "*output_log*.txt" -File | Sort-Object LastWriteTime -Descending | Select-Object -First 1
Add-Content -LiteralPath $latest.FullName -Value 'MJS_RESULT:{"type":"4","externalMatchId":"manual-log-test-001","players":[{"displayName":"TEST_PLAYER","rank":1,"rawScore":25000},{"displayName":"-","rank":2,"rawScore":25000},{"displayName":"-","rank":3,"rawScore":25000},{"displayName":"-","rank":4,"rawScore":25000}]}'
```
