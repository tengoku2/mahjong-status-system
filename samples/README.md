...
作者回答により、個人利用・非商用の範囲で、対局結果データを外部API/DBへ送信・保存することは許可されています。
ただし、TANKIのアセット本体、Prefab、Udonスクリプト、内部構造、内部ロジックの完全再現データは第三者へ共有・配布・公開しないでください。

Botリポジトリに置いてよいのは、TANKI内部名に依存しない中立的な入力形式と、許可された対局結果データを扱うコードだけです。
